...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    test('explicit option', () => {
      const client = new Omnistack({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Omnistack({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Omnistack({ bearerToken: 'My Bearer Token' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['OMNISTACK_API_KEY'] = 'My Bearer Token';
    const client = new Omnistack();
  ...
  test('with overriden environment variable arguments', () => {
    // set options via env var
    process.env['OMNISTACK_API_KEY'] = 'another My Bearer Token';
    const client = new Omnistack({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Omnistack({ bearerToken: 'My Bearer Token', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Omnistack({ bearerToken: 'My Bearer Token', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Omnistack({ bearerToken: 'My Bearer Token', fetch: testFetch });
