...
describe('resource users', () => {
  test('create: only required params', async () => {
    const responsePromise = client.organization.users.create('user_id', { role: 'owner' });
  ...
  test('create: required and optional params', async () => {
    const response = await client.organization.users.create('user_id', { role: 'owner' });
  });
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.organization.users.list({ after: 'after', limit: 0 }, { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Omnistack.NotFoundError);
  });
